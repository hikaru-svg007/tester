...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      {/* 1. TOP HEADER (STICKY) */}
      <div className="px-5 py-4 border-b border-slate-800/80 bg-slate-900/95 backdrop-blur-md flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center space-x-3">
          <div className="relative">
            {activeTab === "chat" && sessions.find(s => s.id === activeSessionId) ? (
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-lg ${getAvatarGradient(sessions.find(s => s.id === activeSessionId)?.avatarUri || "gradient_purple")}`}>
          ...
          <div className="p-4 space-y-5 animate-slideIn">
            <div className="bg-gradient-to-r from-purple-900/40 via-indigo-900/20 to-slate-900 border border-purple-500/20 rounded-2xl p-4 shadow-xl">
              ...
              <button
                ...
                className="mt-3 bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs px-3.5 py-2 rounded-xl border border-purple-400/30 flex items-center gap-1.5 transition-all shadow-lg active:scale-95"
              >
                <span>➕</span> Buat / Impor Karakter
              </button>
            </div>
